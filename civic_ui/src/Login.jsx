...
function Login() {
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      {error && <p className="text-[10px] font-bold text-red-500 bg-red-50 px-3 py-1 rounded-full border border-red-100">{error}</p>}
      <button
        onClick={() => googleLoginHandler()}
        disabled={isLoggingIn}
        className="flex items-center gap-3 bg-white hover:bg-slate-50 text-slate-700 px-6 py-3 rounded-full border border-slate-200 shadow-md transition-all active:scale-95 group font-bold text-sm"
      >
        {isLoggingIn ? (
